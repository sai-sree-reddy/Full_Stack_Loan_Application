...
class GetAllLoanManager extends React.Component{
    constructor(props){
        super(props);
    ...
    render(){
        let allLoan=this.state.loan.map(l=><tr>
            ...
            <td>{l.ltype}</td>
            <td>{l.linterest}</td>
            <td><input type="button"  className="btn btn-outline-primary"  value="Update" onClick={(event)=>{
                this.props.history.push("updateLoanById/"+l.lid)
            }} /> </td>
        </tr>)
        return(
...
export default GetAllLoanManager;